...
Console.WriteLine(LuggageColourCounter.CountParentBags(PuzzleInput.Data));
Console.WriteLine("Part 2");
Console.WriteLine(LuggageColourCounter.CountChildBags(PuzzleInput.Data));

public class LuggageColourCounter
{
    public static int CountParentBags(string input)
    {
        HashSet<string> GetRootParents(Bag bag)
        {
            var bagRoots = new HashSet<string>();

            foreach (var parentBag in bag.ParentBags)
            {
                if (parentBag.ParentBags.Any())
                {
                    bagRoots.UnionWith(GetRootParents(parentBag));
                }

                bagRoots.Add(parentBag.Colour);
            }

            return bagRoots;
        }

        var shinyGold = GetShinyGoldBag(input);

        return GetRootParents(shinyGold)
            .Count();
    }
    
    public static int CountChildBags(string input)
    {
        int GetChildCount(Bag bag)
        {
            var count = 0;
            foreach (var childBag in bag.ChildBags)
            {
                ...
                if (childBag.bag.ChildBags.Any())
                {
                    var childCount = GetChildCount(childBag.bag);
                    count += childBag.count * childCount;
                }
            }
        ...
        return GetChildCount(GetShinyGoldBag(input));
    }

    private static Bag GetShinyGoldBag(string input) 
        => ParseBags(input)
            .Single(x => x.Colour == "shiny gold");

    private static List<Bag> ParseBags(string input)
    {
        var sources = input.Split(Environment.NewLine)
            .Select(arg =>
            {
                var split = arg.Split(" bags contain ");
                var colour = split[0];

                var childColours = split.ElementAtOrDefault(1)
                    ?.Split(", ") ?? Array.Empty<string>();

                var children = childColours.Select(x =>
                {
                    var match = Regex.Match(x, @"(?<count>[\d+]) (?<colour>[a-z]+ [a-z]+) bags?");
                    if (!match.Success)
                    {
                        return null;
                    }
        ...
        var bags = sources.Select(x => x.Colour)
            .Select(colour => new Bag(colour, new List<(Bag, int)>(), new List<Bag>()))
            .ToList();

        foreach (var source in sources.Where(x => x.Children.All(y => y is not null))
            .SelectMany(x => x.Children.Select(y => new {parent = x.Colour, child = y.Colour, count = y.Count})))
        {
            var parent = bags.Single(x => x.Colour == source.parent);
            var child = bags.Single(x => x.Colour == source.child);

            parent.ChildBags.Add((child, source.count));
            child.ParentBags.Add(parent);
        }
...
public record Bag(string Colour, List<(Bag bag, int count)> ChildBags, List<Bag> ParentBags);

public static class PuzzleInput
{
    public const string Data = @"mirrored beige bags contain 1 drab brown bag, 3 dotted crimson bags.
