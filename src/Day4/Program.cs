...
Console.WriteLine(PassportBatchValidator.CountValidPassports(PuzzleInput.Data));
Console.WriteLine("Part 2");
Console.WriteLine(PassportBatchValidator.CountValidPassports2(PuzzleInput.Data));

public static class PassportBatchValidator
{
    ...
    public static long CountValidPassports(string input) =>
        GetPassportsWithValidData(input)
            .Count();

    private static IEnumerable<Dictionary<string, string>> GetPassportsWithValidData(string input) =>
        ParsePassportData(input)
            .Where(passport => RequiredFields.All(x => passport.Keys.Contains(x)));
    ...
    private static bool ValidateIssueYear(string issueYear) =>
        issueYear.Length == 4 && issueYear.All(char.IsDigit)
                              && int.TryParse(issueYear, out var year)
                              && year is >= 2010 and <= 2020;

    private static bool ValidateExpirationYear(string issueYear) =>
        issueYear.Length == 4 && issueYear.All(char.IsDigit)
                              && int.TryParse(issueYear, out var year)
                              && year is >= 2020 and <= 2030;

    private static bool ValidateHeight(string height) =>
        (int.TryParse(height[..^2], out var i), i, height[^2..]) switch
        {
            (true, >=150 and <= 193, "cm") => true,
            (true, >=59 and <= 76, "in") => true,
            _ => false
        };

    private static readonly IReadOnlyDictionary<string, Func<string, bool>> FieldValidationRules
    ...
    private static bool ValidateHairColor(string arg) =>
        arg.Length == 7 && arg.First() == '#'
                        && arg[1..].All(x => x switch
                        {
                            >= 'a' and <= 'f' => true,
                            >= '0' and <= '9' => true,
                            _ => false
    ...
    private static readonly HashSet<string> ValidEyeColors
        = new()
        {
            "amb",
            "blu",
    ...
    private static bool ValidateEyeColor(string arg) =>
        ValidEyeColors.Contains(arg);

    private static bool ValidatePassportId(string arg) => 
        arg.Length == 9 && arg.All(char.IsDigit);

    public static long CountValidPassports2(string input) =>
        GetPassportsWithValidData(input)
            .Count(passport 
                => passport.All(x 
                    => !FieldValidationRules.TryGetValue(x.Key, out var validator)
                       || validator(x.Value)));

    private static IEnumerable<Dictionary<string, string>> ParsePassportData(string input) =>
        input.Split(Environment.NewLine + Environment.NewLine)
            .Select(x
                => x.Split(new[] {" ", Environment.NewLine}, StringSplitOptions.None)
                    .Select(x => x.Split(":"))
...
{
    public const string Data = @"eyr:2029 byr:1931 hcl:z cid:128
