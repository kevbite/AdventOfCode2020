...
{
    public static Heading operator +(Heading a, int b)
        => a with {Value = NormalizeHeading(a.Value + b)};

    public static Heading operator -(Heading a, int b)
        => a with {Value = NormalizeHeading(a.Value - b)};

    private static int NormalizeHeading(int i)
    {
        if (i is < 0)
        {
            i = 360 + i;
...
public record Part1Ship(Heading Heading, (int x, int y) Position)
{
    public Part1Ship ApplyAction(ShipAction action, int value)
    {
        ...
        {
            (ShipAction.North, _) or (ShipAction.Forward, ShipAction.North)
                => this with {Position = (Position.x, Position.y - value)},
            (ShipAction.South, _) or (ShipAction.Forward, ShipAction.South)
                => this with {Position = (Position.x, Position.y + value)},
            (ShipAction.East, _) or (ShipAction.Forward, ShipAction.East)
                => this with {Position = (Position.x + value, Position.y)},
            (ShipAction.West, _) or (ShipAction.Forward, ShipAction.West)
                => this with {Position = (Position.x - value, Position.y)},
            (ShipAction.Left, _)
                => this with {Heading = Heading - value},
            (ShipAction.Right, _)
                => this with {Heading = Heading + value},
            _ => throw new InvalidOperationException()
        };
    ...
    private ShipAction GetHeadingDirection()
    {
        return Heading.Value switch
        {
            0 => ShipAction.North,
            90 => ShipAction.East,
